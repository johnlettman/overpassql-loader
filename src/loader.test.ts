...
describe('loader', () => {
  describe('.loader()', () => {
    ...
    it('should throw Error when callback is invalid', () => {
      expect(() => (loader as any).call({ async: 'z' }, '')).toThrow(
        /callback/
      );
    });
    ...
    for (const i in instanceList) {
      ...
      it(`should callback an Error when content is ${typeof instance}`, () => {
        return new Promise((resolve) => {
          const callback: PsuedoCallback = jest.fn((error) =>
    ...
    it('should callback with stripped comments when options.stripComments is true', () => {
      ...
      return new Promise((resolve) => {
        const callback: PsuedoCallback = jest.fn((error, output) =>
          resolve({ callback, error, output })
        );
        const async = () => callback;
    ...
    it(
      'should callback with stripped whitespace and comments ' +
